...
// 5-point stencil coefficients for heat equation
#define STENCIL_CENTER_COEFF 0.5
#define STENCIL_NEIGHBOR_COEFF 0.125
...
inline void update_plane_inner(const plane_t *old_plane, plane_t *new_plane) {
  ...
  const double c_center = STENCIL_CENTER_COEFF;   // Center weight
  const double c_neigh = STENCIL_NEIGHBOR_COEFF;  // Neighbor weight (1/8)
